...
y_hat_lower <- predict_dist - tau
y_hat_upper <- predict_dist + tau
y_hat_lower
y_hat_upper
```
...
```
```{r}
knn_pred_30_5folds <- kNN(30, 5, train_data)
knn_pred <- sapply(x_train, kNN, k = 3, train_data = braking_data_train)
```
...
data_plot <- ggplot(data = braking_data_train, aes(x = x_train, y = y_train)) +
  ...
  geom_line(data = braking_data_train, aes(x = x_train, y = knn_pred, color = "k-NN Prediction"), size = 1) +
  labs(x = "Speed (km/h)", y = "Braking Distance (m)") +
...
mse_lr <- mean((braking_data_test$dist_m - braking_data_test$predicted_lr)^2)
mse_knn <- mean((braking_data_test$dist_m - braking_data_test$knn_pred)^2)
```
